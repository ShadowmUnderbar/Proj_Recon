...
- [ ] コード変更は日本語コミット＋PR（developベース）。Codexの変更は必ずPR経由
...
GitHub Issue（`.github/ISSUE_TEMPLATE/add-upgrade.yml` のフォーム）で送られた仕様を、開発マシン上のCodexが受け取って本スキルの手順を最後まで実行し、developベースのPRを作るための入口と出口を定義する。対話でユーザーが直接依頼する通常モードと手順本体（パターンA/B/C）は共通で、以下は自律実行時の追加ルール。
...
- **初回のみ**: リポジトリに `add-upgrade` ラベルを作成する（無いとテンプレートのラベルが付かない）。
  ...
  ```
- **起動**: Codex で `/loop 5m` を使い、次の主旨のプロンプトを回す（自己ペースで回す場合は間隔省略）。
...
- ポーラーは **PR作成まで**。マージ・force push はしない。`@Codex` を含むissueには反応しない（そちらはクラウドの `.github/workflows/Codex.yml` の担当）。

> 補足: クラウドの `Codex.yml`（`runs-on: ubuntu-latest`）では Unity Editor が無く、CSVインポート/コンパイル/playtest ができないため、アップグレード追加はローカルポーラー方式を採る。将来 self-hosted runner を用意すれば `@Codex` 起動に一本化する余地はある。
