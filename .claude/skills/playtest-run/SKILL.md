...
- `PointParticleDrop` — 敵撃破時のポイント粒子ドロップ。ドロップ量の分割内訳・撃破時の生成数・接触回収・弾の通過回収（弾が消えないこと）・即着弾での回収・ウェーブ切り替わり時の一括消去・プレイヤーの高さへの追従・取得判定の最小サイズを23項目で検証する。弾の検証は粒子と同じ高さの水平弾道で撃つこと（斜め撃ちは銃口が地面に埋まって弾が即消える）。粒子は専用レイヤー `PointParticle`（8）にあり、レイキャスト側で除外しているため通常のクエリでは掛からない。生成直後の粒子は同じフレームの物理クエリに反映されないので、粒子を出してから撃つまでに必ず1フレーム待つこと。ウェーブ進行の検証は `AddElapsedTime` で実際のTick経路を通すこと（DataStoreの `AdvanceWave()` 直叩きでは一括消去を含む `AdvanceWaveInternal` を通らない）。弾道の検証では `IEnemyDataStore.RemoveAllEnemyData` だけでなく `IEnemyPresenter.RemoveAllEnemies` も呼ぶこと（データだけ消しても敵のビューが残って弾道を塞ぐ）。粒子はプレイヤーの高さへ移動し続けるため、撃つ前に高さが落ち着くまで待つこと
