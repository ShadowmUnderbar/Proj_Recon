...
src=(OUT.parent/'DesignTransfer_20260924/07_preserve_recover.py').read_text(encoding='utf-8-sig');parsed=ast.parse(src);fn=next(n for n in parsed.body if isinstance(n,ast.FunctionDef) and n.name=='fp');exec(compile(ast.Module(body=[fn],type_ignores=[]),'<fingerprint>','exec'))
before=json.loads((OUT/'Before_Audit.json').read_text(encoding='utf-8'));allowed=json.loads((OUT/'04_change_report.json').read_text(encoding='utf-8'));changes=[]
for n,previous in before.items():
 if n not in bpy.data.objects:changes.append(n+' missing');continue
 current=fp(bpy.data.objects[n]);existing=current.get('shapes',{});current['shapes']={k:v for k,v in existing.items() if k not in ['Local_Shoulder_Armhole_20260924','Local_StreetShoe_Form_20260924']} if 'shapes' in current else {}
 if n.startswith('ElseIf_Shoe_') and not previous.get('shapes'):current['shapes'].pop('Basis',None)
 if 'shapes' not in previous:current.pop('shapes',None)
 if n in allowed and n.startswith(('EL_','NaturalPose__EL_')):current['geometry']=previous['geometry']
 if current!=previous:changes.append(n)
report={'protected_changes':changes,'poses':{}}
...
for scene,prefix,colname in [(base,'','ElseIf_Design_Basis'),(natural,'NaturalPose__','ElseIf_Design_NaturalPose')]:
 ...
 for side in ['L','R']:
  volumes=[]
  for part in ['Upper','Sole','AnkleCollar']:
   o=bpy.data.objects['ElseIf_Shoe_'+side+'_'+part];mods=[m for m in o.modifiers if m.type=='SOLIDIFY'];flags=[m.show_viewport for m in mods]
   try:
    for m in mods:m.show_viewport=False
    bpy.context.view_layer.update();s=snap(o);volumes.append(closed(s['vs'],s['polys']))
   finally:
    for m,f in zip(mods,flags):m.show_viewport=f
    bpy.context.view_layer.update()
  ids={g.index for g in bodyob.vertex_groups if 'J_Bip_'+side+'_' in g.name and any(t in g.name for t in ['Foot','Toe'])};points=[p for p,gs in zip(body['vs'],body['groups']) if any(i in ids and w>.1 for i,w in gs)]
  outside=[p for p in points if not any(inside(t,p) for t in volumes)]
  shoe_report[side]={'points':len(points),'outside':len(outside),'sample':[list(p) for p in outside[:8]]}
 report['poses'][scene.name]['shoe_coverage']=shoe_report
...
(OUT/'07_validation.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(report,ensure_ascii=False))
