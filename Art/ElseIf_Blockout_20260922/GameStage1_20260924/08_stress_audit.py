...
OUT=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/GameStage1_20260924');scene=bpy.data.scenes['ElseIf_Game_Validation'];bpy.context.window.scene=scene;rig=bpy.data.objects['ElseIf_Game_Humanoid'];frames=json.loads((OUT/'Pose_Frames.json').read_text());report={}
def snap(o):
...
def inside(t,p):
 ...
 return count%2==1
parts=[o for o in bpy.data.collections['ElseIf_Game'].objects if o.type=='MESH' and o['master_source'].startswith('ElseIf_Jacket_')];bodyob=bpy.data.objects['Game__ElseIf_Mannequin']
for label,frame in frames.items():
 scene.frame_set(frame);bpy.context.view_layer.update();bodies={n:snap(bpy.data.objects['Game__'+n]) for n in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']};cloth={o['master_source']:snap(o) for o in parts};collisions=[]
