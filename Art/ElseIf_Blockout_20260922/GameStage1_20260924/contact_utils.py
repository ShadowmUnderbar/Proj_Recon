...
OUT=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/GameStage1_20260924');scene=bpy.data.scenes['ElseIf_Game_Validation'];rig=bpy.data.objects['ElseIf_Game_Humanoid'];frames=json.loads((OUT/'Pose_Frames.json').read_text());col=bpy.data.collections['ElseIf_Game']
class Surface:
...
def solve(label):
 scene.frame_set(frames[label]);bpy.context.view_layer.update();targets={side:{bn:list(rig.pose.bones['J_Bip_'+side+'_'+bn].rotation_quaternion) for bn in ['UpperArm','LowerArm','Hand']} for side in ['L','R']};bodies=[snap(bpy.data.objects['Game__'+n])[1] for n in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']]
