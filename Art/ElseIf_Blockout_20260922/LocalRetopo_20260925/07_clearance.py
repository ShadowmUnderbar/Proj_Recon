...
OUT=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/LocalRetopo_20260925');scene=bpy.data.scenes['ElseIf_Topology_Review'];rig=bpy.data.objects['ElseIf_Topology_Humanoid'];frames=json.loads((OUT/'Pose_Frames.json').read_text());col=bpy.data.collections['ElseIf_Game_LocalRetopo']
class Surface:
...
def solve(label):
 scene.frame_set(frames[label]);bpy.context.view_layer.update();targets={side:{bn:list(rig.pose.bones['J_Bip_'+side+'_'+bn].rotation_quaternion) for bn in ['UpperArm','LowerArm','Hand']} for side in ['L','R']};bodies=[snap(bpy.data.objects['Topo__'+n])[1] for n in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']]
 ...
 for o in cloth:
  ...
  pair={side:o.shape_key_add(name='DLHN_NewTopology_Clearance_'+side+'_'+label,from_mix=False) for side in ['L','R']}
 ...
 for side in ['L','R']:
  ...
  for o in col.objects:
   ...
   if updates:
    if not o.data.shape_keys:o.shape_key_add(name='Basis')
    k=o.shape_key_add(name='DLHN_NewTopology_Clearance_'+side+'_'+label,from_mix=False)
...
for label in ['A_Natural','B_Side90','C_Forward','D_AsymmetricAim','F_OneArmBack','G_WideOpen','H_FrontNear','I_FrontBack','K_Arms45']:solve(label)
scene.frame_set(1)
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'ElseIf_LocalRetopo.blend'))
