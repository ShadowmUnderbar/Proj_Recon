...
P=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/LocalRetopo_20260925');scene=bpy.data.scenes['ElseIf_Topology_Review'];bpy.context.window.scene=scene;scene.frame_set(1);col=bpy.data.collections['ElseIf_Game_LocalRetopo'];rig=bpy.data.objects['ElseIf_Topology_Humanoid'];frames=json.loads((P/'Pose_Frames.json').read_text())
# 削除判定は承認済み衣装変形で行い、修正後にも削除境界を再検証。
for o in col.objects:
 if o.type!='MESH' or o['master_source'] in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']:continue
 o.data=bpy.data.objects['Opt__'+o['master_source']].data.copy()
...
bodies=[bpy.data.objects['Topo__'+n] for n in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']];clothes=[o for o in col.objects if o.type=='MESH' and o['master_source'].startswith(('ElseIf_Jacket_','ElseIf_Shorts','ElseIf_Sock_','ElseIf_Shoe_'))]
def snap(o):
...
for o in bodies:
 ...
 for v in o.data.vertices:
  ...
  border=any(abs(p.z-z)<width for z,width in [(.727,.035),(.920,.025),(.216,.03),(1.075,.025)])
  if hand or (p.z>1.325 and abs(p.x)<.105) or (p-sh).length<.065 or (p-wrist).length<.052 or border:protected.add(v.index)
...
for label,frame in frames.items():
 ...
 (P/'Hidden_Progress.json').write_text(json.dumps({'pose':label,'faces':{n:len(v) for n,v in candidate.items()}}))
...
for o in bodies:
 ...
 # 衣服境界と不確実領域から、人体側へ3面リング残す。
 for _ in range(3):selected={i for i in selected if all(j in selected for j in adj[i])}
...
(P/'Body_Deletion_Record.json').write_text(json.dumps(maps));(P/'Body_Deletion_Counts.json').write_text(json.dumps(result,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(P/'ElseIf_LocalRetopo.blend'));print(result)
