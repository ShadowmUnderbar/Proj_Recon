...
def solve(label):
 ...
 for o in cloth:
  ...
  pair={}
  for side in ['L','R']:
   name='DLHN_NewTopology_Clearance_'+side+'_'+label;k=o.data.shape_keys.key_blocks.get(name) or o.shape_key_add(name=name,from_mix=False);k.driver_remove('value');pair[side]=k
 ...
 for side in ['L','R']:
  ...
  for o in col.objects:
   ...
   if updates:
    if not o.data.shape_keys:o.shape_key_add(name='Basis')
    name='DLHN_NewTopology_Clearance_'+side+'_'+label;k=o.data.shape_keys.key_blocks.get(name) or o.shape_key_add(name=name,from_mix=False)
