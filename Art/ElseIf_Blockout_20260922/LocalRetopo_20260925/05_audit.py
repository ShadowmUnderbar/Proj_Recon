...
OUT=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/LocalRetopo_20260925');scene=bpy.data.scenes['ElseIf_Topology_Review'];bpy.context.window.scene=scene;rig=bpy.data.objects['ElseIf_Topology_Humanoid'];frames=json.loads((OUT/'Pose_Frames.json').read_text());report={}
def snap(o):
...
def inside(t,p):
 ...
 return count%2==1
parts=[o for o in bpy.data.collections['ElseIf_Game_LocalRetopo'].objects if o.type=='MESH' and o['master_source'].startswith('ElseIf_Jacket_')];bodyob=bpy.data.objects['Topo__ElseIf_Mannequin']
for label,frame in frames.items():
 scene.frame_set(frame);bpy.context.view_layer.update();bodies={n:snap(bpy.data.objects['Topo__'+n]) for n in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']};cloth={o['master_source']:snap(o) for o in parts};collisions=[]
 ...
 for side in ['L','R']:
  ...
  for n in ['ElseIf_Jacket_Sleeve_'+side,'ElseIf_Jacket_Cuff_'+side]:
   item=cloth[n];attrs=bpy.data.objects['Topo__'+n].data.attributes['DLHN_OuterSurface'];outer={i for i,a in enumerate(attrs.data) if a.value>.999};off=len(vs);vs+=item[0];fs += [tuple(off+i for i in f) for f in item[2] if all(i in outer for i in f)]
