...
P=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/OptimizationTest_20260925');s=bpy.data.scenes['ElseIf_Optimized_Review'];bpy.context.window.scene=s;s.frame_set(1);col=bpy.data.collections['ElseIf_Game_Optimized_Test'];report=[]
def priority(n,p):
 z=p.z;x=abs(p.x)
...
for o in col.objects:
 ...
 bmesh.ops.dissolve_limit(bm,angle_limit=math.radians(1.4),use_dissolve_boundaries=False,verts=low,edges=edges,delimit={'MATERIAL','SEAM','SHARP','UV'})
...
(P/'Regional_Reduction.json').write_text(json.dumps(report,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(P/'ElseIf_Optimization_Test.blend'));print('Total triangles',sum(r['before_triangles'] for r in report),sum(r['after_triangles'] for r in report));print([r for r in report if r['before_triangles']!=r['after_triangles']])
