...
OUT=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/OptimizationTest_20260925');scene=bpy.data.scenes['ElseIf_Optimized_Review'];rig=bpy.data.objects['ElseIf_Optimized_Humanoid'];frames=json.loads((OUT/'Pose_Frames.json').read_text());col=bpy.data.collections['ElseIf_Game_Optimized_Test']
class Surface:
...
def solve(label):
 scene.frame_set(frames[label]);bpy.context.view_layer.update();targets={side:{bn:list(rig.pose.bones['J_Bip_'+side+'_'+bn].rotation_quaternion) for bn in ['UpperArm','LowerArm','Hand']} for side in ['L','R']};bodies=[snap(bpy.data.objects['Opt__'+n])[1] for n in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']]
...
for label in ["B_Side90","G_WideOpen"]:solve(label)
scene.frame_set(1)
