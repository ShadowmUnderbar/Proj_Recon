...
OUT=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/LOD0Candidate_20260925');scene=bpy.data.scenes['ElseIf_LOD0_Review'];bpy.context.window.scene=scene;rig=bpy.data.objects['ElseIf_LOD0_Humanoid'];frames=json.loads((OUT/'Pose_Frames.json').read_text());report={}
def snap(o):
...
def inside(t,p):
 ...
 return count%2==1
components={int(k):v for k,v in json.loads((OUT/'Component_Map.json').read_text()).items()}
def split_clothes():
 result={}
 for o in bpy.data.collections['ElseIf_Game_Optimized'].objects:
  if o.type!='MESH' or o.name in ['LOD0__ElseIf_Mannequin','LOD0__ElseIf_Anatomy_Restoration']:continue
  e=o.evaluated_get(bpy.context.evaluated_depsgraph_get());m=e.to_mesh();ca=m.attributes['DLHN_Component'];va=m.attributes['DLHN_ComponentVertex'];bucket={}
  for v in m.vertices:
   cid=ca.data[v.index].value;n=components[cid]['source']
   if not n.startswith(('ElseIf_Jacket_','ElseIf_Sock_','ElseIf_Shoe_','ElseIf_Shorts')):continue
   if cid not in bucket:bucket[cid]={'v':[None]*components[cid]['vertices'],'f':[]}
   bucket[cid]['v'][va.data[v.index].value]=o.matrix_world@v.co
  for f in m.polygons:
   cid=ca.data[f.vertices[0]].value
   if cid in bucket:bucket[cid]['f'].append(tuple(va.data[i].value for i in f.vertices))
  for cid,b in bucket.items():
   vs=b['v'];polys=b['f'];result[components[cid]['source']]=(vs,[],polys,[],BVHTree.FromPolygons(vs,polys))
  e.to_mesh_clear()
 return result
bodyob=bpy.data.objects['LOD0__ElseIf_Mannequin']
for label,frame in frames.items():
 scene.frame_set(frame);bpy.context.view_layer.update();bodies={n:snap(bpy.data.objects['LOD0__'+n]) for n in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']};cloth=split_clothes();collisions=[]
 for name,item in cloth.items():
  for bn,body in bodies.items():
   pairs=item[4].overlap(body[4])
   if pairs:
    samples=[sum((item[0][i] for i in item[2][a]),Vector())/len(item[2][a]) for a,b in pairs];collisions.append({'cloth':name,'body':bn,'pairs':len(pairs),'bounds':[[min(p[i] for p in samples),max(p[i] for p in samples)] for i in range(3)]})
 ...
 (OUT/'Optimized_Final_Stress_Audit.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
