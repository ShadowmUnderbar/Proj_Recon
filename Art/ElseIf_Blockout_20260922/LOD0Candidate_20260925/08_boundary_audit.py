...
P=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/LOD0Candidate_20260925');s=bpy.data.scenes['ElseIf_LOD0_Review'];bpy.context.window.scene=s;col=bpy.data.collections['ElseIf_Game_Optimized'];records=json.loads((P/'Body_Deletion_Record.json').read_text());frames=json.loads((P/'Pose_Frames.json').read_text());dirs=[Vector((x,y,z)).normalized() for x in [-1,0,1] for y in [-1,0,1] for z in [-1,0,1] if x or y or z]
def snap(o):
...
for name,r in records.items():
 ...
 bins={}
 src=bpy.data.objects['Backup__'+o['master_source']]
 for fi in deleted:
