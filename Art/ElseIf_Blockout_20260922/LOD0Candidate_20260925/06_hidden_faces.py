...
P=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/LOD0Candidate_20260925');scene=bpy.data.scenes['ElseIf_LOD0_Review'];bpy.context.window.scene=scene;scene.frame_set(1);col=bpy.data.collections['ElseIf_Game_Optimized'];rig=bpy.data.objects['ElseIf_LOD0_Humanoid'];frames=json.loads((P/'Pose_Frames.json').read_text())
bodies=[bpy.data.objects['LOD0__'+n] for n in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']];clothes=[o for o in col.objects if o.type=='MESH' and o['master_source'].startswith(('ElseIf_Jacket_','ElseIf_Shorts','ElseIf_Sock_','ElseIf_Shoe_'))]
def snap(o):
...
for o in bodies:
 ...
 for v in o.data.vertices:
  ...
  if hand or (p.z>1.325 and abs(p.x)<.105) or (p-sh).length<.045 or (p-wrist).length<.052 or border:protected.add(v.index)
...
(P/'Body_Deletion_Record.json').write_text(json.dumps(maps));(P/'Body_Deletion_Counts.json').write_text(json.dumps(result,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(P/'ElseIf_Game_LOD0_Candidate.blend'));print(result)
