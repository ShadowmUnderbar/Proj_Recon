...
P=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/LOD0Candidate_20260925');frames=json.loads((P/'Pose_Frames.json').read_text());cur=bpy.data.scenes['ElseIf_Game_Backup_Review'];opt=bpy.data.scenes['ElseIf_LOD0_Review'];col=bpy.data.collections['ElseIf_Game_Optimized'];rig=bpy.data.objects['ElseIf_LOD0_Humanoid'];report={}
fn=next(n for n in ast.parse((P/'07_regional_reduce.py').read_text(encoding='utf-8-sig')).body if isinstance(n,ast.FunctionDef) and n.name=='priority');exec(compile(ast.Module(body=[fn],type_ignores=[]),'<priority>','exec'))
def snap(o):
...
pairs=[(bpy.data.objects['Backup__'+o['master_source']],o) for o in col.objects if o.type=='MESH' and not any(t in o['master_source'] for t in ['Mannequin','Anatomy_Restoration']) and len(o.data.vertices)!=len(bpy.data.objects['Backup__'+o['master_source']].data.vertices)]
for label,frame in frames.items():
 ...
 key=bpy.data.objects['LOD0__ElseIf_Jacket_Back'].data.shape_keys;activation={side:key.key_blocks['DLHN_Wide_ClothEase_'+side].value for side in ['L','R']}
