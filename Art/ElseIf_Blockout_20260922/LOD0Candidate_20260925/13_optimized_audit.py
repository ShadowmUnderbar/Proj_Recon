...
def inside(t,p):
 ...
 return count%2==1
parts=[o for o in bpy.data.collections['ElseIf_Game_Optimized'].objects if o.type=='MESH' and o['master_source'].startswith('ElseIf_Jacket_')];bodyob=bpy.data.objects['LOD0__ElseIf_Mannequin']
for label,frame in frames.items():
 scene.frame_set(frame);bpy.context.view_layer.update();bodies={n:snap(bpy.data.objects['LOD0__'+n]) for n in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']};cloth={o['master_source']:snap(o) for o in parts};collisions=[]
 ...
 (OUT/'Optimized_PreMerge_Stress_Audit.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
