...
edgecounts={};geo={v.index:tuple(round(c,5) for c in v.co) for v in m.vertices}
for f in m.polygons:
 for a,b in f.edge_keys:edgecounts[tuple(sorted((geo[a],geo[b])))]=edgecounts.get(tuple(sorted((geo[a],geo[b]))),0)+1
boundarycoords={p for e,c in edgecounts.items() if c==1 for p in e};boundary={i for i,p in geo.items() if p in boundarycoords}
for v in m.vertices:
 keep=v.index in boundary or (1.16<v.co.z<1.23 and .075<abs(v.co.x)<.14) or v.co.z>1.32 or any(g.weight>.001 and any(x in o.vertex_groups[g.group].name for x in ['Hand','Thumb','Index','Middle','Ring','Little']) for g in v.groups)
...
r={'before_triangles':before,'after_triangles':after,'protected_points':len(protected),'max_protected_vertex_distance_mm':mx*1000};(P/'Body_Density_Final.json').write_text(json.dumps(r,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(P/'ElseIf_Game_MobileVR_Test.blend'));print(r)
