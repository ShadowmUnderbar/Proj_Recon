...
frames=[1,11,41,51,61,71,91,101];bad={n:set() for n in ['Jacket']};log={}
for frame in frames:
 ...
 # 衣服・人体の交差面にも限定して密度を戻す。
 j=snap(bpy.data.objects['Mobile__Jacket']);jo=bpy.data.objects['Mobile__Jacket'];jo.data.calc_loop_triangles();jp=[t.polygon_index for t in jo.data.loop_triangles]
 for bn in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']:
  bo=bpy.data.objects['Mobile__'+bn];bb=snap(bo);bo.data.calc_loop_triangles();bp=[t.polygon_index for t in bo.data.loop_triangles]
  for ji,bi in j['tree'].overlap(bb['tree']):
   bad['Jacket'].add(jp[ji])
   if bn in bad:bad[bn].add(bp[bi])
...
for n,ids in bad.items():
 ...
 for v in m.vertices:
  ...
  weights=[1-u-vv,u,vv];ii.append(tri);ww.append(weights);coords.append(list(v.co));groups={}
...
(P/'Adaptive_Refinement_Final.json').write_text(json.dumps(log,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(P/'ElseIf_Game_MobileVR_Test.blend'));print(log)
