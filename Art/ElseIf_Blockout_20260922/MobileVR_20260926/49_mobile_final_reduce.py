...
def ratio(n):
 if n.startswith('ElseIf_Jacket_'):
  ...
  if 'Cuff' in n:return .30
  if 'Collar' in n:return .12
  if 'Hem' in n:return .18
  return .23
...
for original in list(col.objects):
 if original.name!='Mobile__Jacket':continue
 ...
 keyarrays={k.name:np.array([list(v.co) for v in k.data],dtype=np.float32) for k in keys};sensitive=set()
 if keys:
  ee=np.array([list(e.vertices) for e in m.edges]);peak=np.zeros(len(ee))
  for k in keys[1:]:
   delta=keyarrays[k.name]-keyarrays[keys[0].name];peak=np.maximum(peak,np.linalg.norm(delta[ee[:,0]]-delta[ee[:,1]],axis=1))
  sensitive=set(ee[peak>.005].ravel().tolist())
  for step in range(1):
   sensitive |= {int(i) for e in ee if int(e[0]) in sensitive or int(e[1]) in sensitive for i in e}
 allcoords=np.array([list(v.co) for v in m.vertices],dtype=np.float32)
 for cid in cids:
  ...
  for v in mesh.vertices:
   ...
   if 'Sleeve' in name:
    if p.z>1.22 or p.z<1.00:w=min(w,.12)
   if 'Sock' in name and (.46<p.z<.60 or p.z<.27):w=min(w,.15)
   if name.startswith('ElseIf_Jacket_') and (p.z>1.19 or p.z<.865):w=min(w,.12)
   if ids[v.index] in sensitive:w=0.0
   protect.add([v.index],w,'REPLACE')
  if before>100:
   # Shape Keyのない作業部品にだけ局所Decimateを適用。後で元部品から補正量を補間する。
   dec=part.modifiers.new('Regional_Mobile_Reduction','DECIMATE');dec.ratio=min(.95,max(rate,.10+.90*sum(ids[v.index] in sensitive for v in mesh.vertices)/len(mesh.vertices)));dec.vertex_group=protect.name;dec.vertex_group_factor=8.0;dec.use_collapse_triangulate=True
  ...
  (P/'Mobile_Reduction_Final.json').write_text(json.dumps(report,indent=2))
 # 全部品が同じキー一覧を持つ状態で統合。
