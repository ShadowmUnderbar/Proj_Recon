...
for v in m.vertices:
 keep=v.co.z>1.32 or any(g.weight>.001 and any(x in o.vertex_groups[g.group].name for x in ['Hand','Thumb','Index','Middle','Ring','Little']) for g in v.groups)
...
r={'before_triangles':before,'after_triangles':after,'protected_points':len(protected),'max_protected_vertex_distance_mm':mx*1000};(P/'Body_Density_Reduction.json').write_text(json.dumps(r,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(P/'ElseIf_Game_MobileVR_Test.blend'));print(r)
