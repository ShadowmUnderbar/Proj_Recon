...
bodies=[bpy.data.objects['Mobile__ElseIf_Anatomy_Restoration']];clothes=[o for o in col.objects if o.type=='MESH' and 'Mannequin' not in o.name and 'Anatomy' not in o.name]
def snap(o):
...
for label,frame in frames.items():
 ...
 vs0,fs0=snap(bpy.data.objects['Mobile__ElseIf_Mannequin']);off=len(cv);cv+=vs0;cf.extend(tuple(i+off for i in f) for f in fs0)
...
(P/'Body_Overlap_Deletion_Record.json').write_text(json.dumps(maps));(P/'Body_Overlap_Deletion_Counts.json').write_text(json.dumps(result,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(P/'ElseIf_Game_MobileVR_Test.blend'));print(result)
