...
for original in list(col.objects):
 ...
 for cid in cids:
  ...
  for v in mesh.vertices:
   w=1.0;p=v.co
   if v.index in boundary:w=.02
   if 'Sleeve' in name:
    if p.z>1.28 or 1.14<p.z<1.18 or p.z<.92:w=0.0
   if 'Sock' in name and (.515<p.z<.555 or .22<p.z<.25):w=0.0
   if name.startswith('ElseIf_Jacket_') and (p.z>1.29 or (1.07<p.z<1.21 and abs(p.x)<.19)):w=0.0
  ...
  (P/'Regional_Reduction_Final.json').write_text(json.dumps(report,indent=2))
 # 全部品が同じキー一覧を持つ状態で統合。
