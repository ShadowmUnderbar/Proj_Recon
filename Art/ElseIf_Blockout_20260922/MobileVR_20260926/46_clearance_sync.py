...
for v,k in zip(m.vertices,m.shape_keys.key_blocks[0].data):v.co=k.co
m.update()
  ...
  for bn in ['ElseIf_Mannequin','ElseIf_Anatomy_Restoration']:
   ...
   for fi,bi in pairs:
    for vi in a['f'][fi]:
     ...
     if not hit:continue
     tri=b['f'][bi];pa,pb,pc=[b['v'][i] for i in tri];n=(pb-pa).cross(pc-pa).normalized();worlddelta=n*.001;matrix=Matrix(((0,0,0),(0,0,0),(0,0,0)))
...
s.frame_set(1);(P/'Local_Clearance_Synchronized.json').write_text(json.dumps(logs,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(P/'ElseIf_Game_MobileVR_Test.blend'));print(json.dumps(logs))
