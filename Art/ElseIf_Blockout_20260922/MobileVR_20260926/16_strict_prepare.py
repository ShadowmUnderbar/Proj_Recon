...
P=Path('D:/UnityProj/Proj_Recon/Art/ElseIf_Blockout_20260922/MobileVR_20260926');s=bpy.data.scenes['ElseIf_MobileVR_Review'];bpy.context.window.scene=s;s.frame_set(1);bpy.ops.wm.save_as_mainfile(filepath=str(P/'Before_Strict_Protection.blend'),copy=True)
...
for src in bpy.data.collections['ElseIf_Game_Optimized'].objects:
 if src.type!='MESH' or 'Mannequin' in src.name or 'Anatomy' in src.name:continue
