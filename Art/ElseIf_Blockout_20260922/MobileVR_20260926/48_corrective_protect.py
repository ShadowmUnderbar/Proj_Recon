...
for original in list(col.objects):
 ...
 if keys:
  ...
  sensitive=set(ee[peak>.0025].ravel().tolist())
  for step in range(2):
 ...
 for cid in cids:
  ...
  if before>100:
   ...
   dec=part.modifiers.new('Regional_Mobile_Reduction','DECIMATE');dec.ratio=min(.95,rate+(1-rate)*sum(ids[v.index] in sensitive for v in mesh.vertices)/len(mesh.vertices));dec.vertex_group=protect.name;dec.vertex_group_factor=8.0;dec.use_collapse_triangulate=True
  ...
  (P/'Corrective_Protected_Reduction.json').write_text(json.dumps(report,indent=2))
 # 全部品が同じキー一覧を持つ状態で統合。
