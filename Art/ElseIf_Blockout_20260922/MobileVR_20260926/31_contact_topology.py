...
s.frame_set(1);bm=bmesh.new();bm.from_mesh(m);bm.faces.ensure_lookup_table();oldverts=set(bm.verts);edges={e for i in bad for e in bm.faces[i].edges};bmesh.ops.subdivide_edges(bm,edges=list(edges),cuts=2,use_grid_fill=True);bmesh.ops.triangulate(bm,faces=list(bm.faces));layer=bm.verts.layers.int.get('Mobile_ContactNew') or bm.verts.layers.int.new('Mobile_ContactNew')
for v in bm.verts:v[layer]=0 if v in oldverts else 1
...
for v in m.vertices:
 ...
 count+=1
m.update();(P/'Contact_Retopology.json').write_text(json.dumps({'faces_refined':len(bad),'vertices_added':count},indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(P/'ElseIf_Game_MobileVR_Test.blend'));print({'faces_refined':len(bad),'vertices_added':count})
